  ...
  const links = [
    {
      ...
      icon: (
        <FaRegUserCircle
          className={`icon fs-2 ${pathname.includes('/Kanbas/Account') ? 'text-danger' : 'text-white'}`}
        />
      ),
    },
  ...
  return (
    <ul className="wd-kanbas-navigation d-none d-sm-block">
      ...
      {links.map((link, index) => (
        <li
          key={index}
          className={`nav-item ${pathname.includes(`/Kanbas/${link.label}`) ? 'active' : ''}`}
        >
          <Link
          ...
          >
            {link.icon}
            <span
              className={`link-text ${pathname.includes(`/Kanbas/${link.label}`) ? 'text-danger' : 'text-white'}`}
            >
              {link.label}
            </span>
          </Link>
